...
launch_tmux_service() {
    ...
    if [ -n "$target_host" ]; then
        host_env="HERD_HOST=$target_host "
    fi
    ...
    tmux new-session -d -s "$target_session" -c "$APP_DIR" \
        "echo \"Build: $LAUNCH_COMMIT ($LAUNCH_BRANCH) @ $LAUNCH_TIME\" && \
         ...
         while true; do \
           set -o pipefail; \
           ${host_env}NODE_ENV=$NODE_ENV_VALUE PORT=$target_port HERD_BACKGROUND_RUNTIMES=$target_background_runtimes HERD_STOP_ACTIVE_SESSIONS_ON_BOOT=$target_stop_active_sessions_on_boot LAUNCH_COMMIT=$LAUNCH_COMMIT $RUN_COMMAND 2>&1 | tee -a \"$LAUNCH_LOG_FILE\"; \
...
DB_READY_LOG="$(mktemp "${TMPDIR:-/tmp}/herd-db-ready.XXXXXX")"
