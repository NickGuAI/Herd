...
function printUsage(stdout: Writable): void {
  stdout.write('Usage:\n')
  stdout.write('  herd sessions list [--type commander|worker|automation|all]\n')
  stdout.write('  herd sessions info <name> [--tail <N>] [--json]\n')
  stdout.write('  herd session register --name <name> --machine <machine> [--cwd <path>] [--agent <provider>] [--task <text>]\n')
  stdout.write('  herd session heartbeat --name <name>\n')
  stdout.write('  herd session events --name <name> --events \'[{"type":"..."}]\'\n')
  stdout.write('  herd session unregister --name <name>\n')
}
...
export async function runSessionCli(
  ...
  if (!config) {
    stderr.write('Not configured. Run: herd onboard\n')
    return 1
  }
