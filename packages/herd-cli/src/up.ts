...
function printUpUsage(write: (chunk: string) => void): void {
  write('Usage: herd up [--dev] [--port <port>]\n')
  write('\n')
...
export function resolveLaunchScript(appDir: string): string {
  return path.resolve(appDir, '../../operations/scripts/launch_herd.sh')
}
...
function runManagedLaunch(plan: LaunchPlan): number {
  ...
  if (result.error) {
    process.stderr.write(
      `herd up --dev: failed to launch managed starter: ${result.error.message}\n`,
    )
    return 1
  }
...
function runForegroundLaunch(plan: LaunchPlan): Promise<number> {
  ...
  return new Promise<number>((resolve) => {
    ...
    child.on('error', (err) => {
      process.stderr.write(
        `herd up: failed to start pnpm: ${err.message}\n`,
      )
      resolve(1)
    })
  })
...
export async function runUpCli(args: readonly string[]): Promise<number> {
  ...
  if (parsed.error) {
    process.stderr.write(`${formatStatusLine('fail', 'herd up', parsed.error, 'Run herd up --help for supported options.')}\n`)
  ...
  if (!appDir) {
    process.stderr.write(
      `${formatStatusLine(
        ...
        'Run apps/herd/install.sh once, or export HERD_APP_DIR.',
      )}\n`,
    )
    return 1
  }
  ...
  for (const failure of telemetrySync.failed) {
    process.stderr.write(
      `herd up: warning: failed to sync ${failure.agent} telemetry: ${failure.error.message}\n`,
    )
  }
