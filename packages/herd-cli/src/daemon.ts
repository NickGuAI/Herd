...
function normalizeSpawnEnv(
  ...
  const approvalBase = env.HERD_APPROVAL_BASE_URL?.trim()
  if (!approvalBase || /^https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?/u.test(approvalBase)) {
    env.HERD_APPROVAL_BASE_URL = normalizeEndpoint(endpoint)
  }
  return env
}
...
export async function runDaemonCli(
  ...
  const configuredReconnectDelayMs = Number.parseInt(env.HERD_DAEMON_RECONNECT_MS ?? '', 10)
  ...
  return await new Promise<number>((resolve) => {
    ...
    function connect(): void {
      const ws = new WebSocketImpl(websocketUrl, ['herd-daemon', runOptions.pairingToken])
      activeWs = ws
