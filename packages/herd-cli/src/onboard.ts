...
const AGENT_INSTRUCTIONS: readonly AgentInstruction[] = [
  {
    ...
    lines: [
      ...
      '  OTEL_EXPORTER_OTLP_HEADERS=x-herd-api-key=<KEY>',
    ],
  },
  {
    id: 'codex',
    ...
    lines: [
      ...
      '  exporter = { otlp-http = { endpoint = "<endpoint>/v1/logs", protocol = "json", headers = { "x-herd-api-key" = "<KEY>" } } }',
    ],
  },
  {
    id: 'terminal-cri',
    label: 'Terminal CRI',
    lines: ['Already integrated. Herd agents read ~/.herd.json directly; no extra setup is required.'],
  },
  {
    id: 'cursor',
    label: 'Cursor',
...
function printUsage(): void {
  process.stdout.write(
    'Usage: herd onboard [--non-interactive --endpoint <url> --api-key <key> --agents <agent,...>] [--skip-founder-operator] [--skip-tailscale]\n',
  )
}
...
function parseAgentSelection(value: string): HerdAgent[] | null {
  const agents = value
    .split(',')
  ...
  for (let index = 0; index < rawArgs.length; index += 1) {
    ...
    if (arg === '--agents' || arg.startsWith('--agents=')) {
      ...
      if (!agents) {
        return {
          ...options,
          error: `--agents must be a comma-separated list of: ${HERD_AGENTS.join(', ')}`,
        }
      }
...
function defaultOperatorsPath(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveHerdDataDir(env), 'operators.json')
}
...
async function runTailscaleSetup(
  ...
  process.stdout.write('  herd machine onboard --from-tailscale-status --cwd <absolute-workspace-path>\n')
...
function printSelectedAgentInstructions(
  ...
  autoConfigured: ReadonlySet<HerdAgent>,
): void {
  ...
  if (hasManualAgents) {
    ...
    process.stdout.write('OTEL_EXPORTER_OTLP_HEADERS=x-herd-api-key=<your-api-key>\n')
  }
}
...
function printProviderRuntimeInstructions(providers: readonly ProviderRegistryEntry[]): void {
  ...
  process.stdout.write('- For subscription/OAuth CLI auth on a personal machine, pair the machine from Herd Machines or run `herd machine daemon-pair --machine <id>`, then run the printed `herd daemon run ...` command on that machine.\n')
  process.stdout.write('- Check daemon readiness with `herd machine daemon-status --machine <id>`.\n')
...
async function resolveAgents(options: ParsedOnboardArgs, setupPath: SetupPath): Promise<HerdAgent[]> {
  if (options.agents) {
  ...
  return promptMultiSelect<HerdAgent>(
...
function printConfigurationSavedSummary(input: {
  ...
  process.stdout.write('\nNext commands\n')
  process.stdout.write('  herd doctor\n')
  process.stdout.write('  herd up\n')
...
export async function runCli(
  ...
  try {
      ...
      process.stderr.write(
        `${formatStatusLine(
          ...
          validation.validationUrl
            ? `Retry after confirming ${validation.validationUrl} accepts x-herd-api-key.`
            : 'Confirm the endpoint and API key, then rerun herd onboard.',
        )}\n`,
      )
      return 1
    }
    ...
    const config = createHerdConfig({
      endpoint,
      apiKey,
      agents,
    })

    await writeHerdConfig(config)
    ...
    const autoConfigured = new Set<HerdAgent>()
    ...
    process.stdout.write(`\n${formatStatusLine('pass', 'Onboarding complete', 'run herd doctor, then open the browser onboarding guide')}\n`)
