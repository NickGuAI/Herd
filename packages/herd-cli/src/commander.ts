...
import { type HerdConfig, readHerdConfig } from './config.js'
...
#
# max agent turns now default from ~/.herd/config.yaml (default: 300)
# per-commander maxTurns overrides belong in Herd Settings/UI or the create API, not COMMANDER.md frontmatter
#
...
\`\`\`
herd quests list
herd quests claim <quest-id>
herd quests note <quest-id> "<progress text>"
herd quests done <quest-id> --note "<summary>"
\`\`\`
...
\`\`\`
herd workers list
herd workers dispatch --session [COMMANDER_ID] [--task "<initial task>"] [--cwd <path>] [--machine <id>] [--permission-mode <default|acceptEdits|bypassPermissions>]
herd workers status <session-name>
herd workers send <session-name> "<task>"
herd workers kill <session-name>
\`\`\`
...
Use \`herd workers send <name> "<task>"\` to message existing named sessions.
...
\`\`\`
herd automation list --commander [COMMANDER_ID]
\`\`\`
...
Indexed commander transcript search is a Herd runtime feature.
...
- Commander memory search/recollection is still not a Herd runtime feature.
...
function printUsage(stdout: Writable): void {
  stdout.write('Usage:\n')
  stdout.write('  herd commander init\n')
  stdout.write('  herd commander init --remote <server-url> --token <sync-token> [--commander <id>] [--poll-interval <seconds>] [--once]\n')
  stdout.write('  herd commander load <path>\n')
  stdout.write('  herd commander transcripts search --commander <id> "<query>" [--top-k <count>]\n')
}
...
async function runRemoteInit(
  ...
  const commanderId = parseNonEmpty(
    commanderIdArg ?? deps.commanderId ?? process.env.HERD_COMMANDER_ID,
  )
  if (!commanderId) {
    stderr.write('--commander or HERD_COMMANDER_ID is required for remote init.\n')
    return 1
  }
...
async function runTemplateLoad(
  ...
  const stderr = deps.stderr ?? process.stderr
  const fetchImpl = deps.fetchImpl ?? fetch
  const readConfig = deps.readConfig ?? readHerdConfig
