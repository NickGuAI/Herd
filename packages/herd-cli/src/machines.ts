...
function printUsage(stdout: Writable): void {
  stdout.write('Usage:\n')
  stdout.write('  herd machine list\n')
  stdout.write('  herd machine add --id <id> --label <label> (--host <host> | --tailscale-hostname <hostname>) [--user <user>] [--port <port>] [--cwd <cwd>] [--verify-launch]\n')
  stdout.write('  herd machine onboard --from-tailscale-status [--id <id>] [--label <label>] [--user <user>] [--port <port>] --cwd <cwd>\n')
  stdout.write('  herd machine check <id>\n')
  stdout.write('  herd machine verify-launch --machine <id> [--provider <provider>]\n')
  stdout.write('  herd machine remove <id>\n')
  stdout.write('  herd machine bootstrap <id> [--tools <provider-a,provider-b>] [--skip-telemetry]\n')
  stdout.write('  herd machine auth-status --machine <id>\n')
  stdout.write('  herd machine auth-setup --machine <id> --provider <provider> [--mode <setup-token|api-key|device-auth>] [--secret <value>]\n')
  stdout.write('  herd machine daemon-pair --machine <id> [--label <label>] [--cwd <cwd>]\n')
  stdout.write('  herd machine daemon-status --machine <id>\n')
  stdout.write('  herd machine daemon-revoke --machine <id>\n')
}
...
function buildBootstrapScript(
  ...
  const scriptLines = [
    'set -eu',
    'PREFIX="$HOME/.herd/tools"',
    'BIN_DIR="$PREFIX/bin"',
    'ensure_path_block() {',
    '  file="$1"',
    '  touch "$file"',
    '  if ! grep -Fq \'export PATH="$HOME/.herd/tools/bin:$PATH"\' "$file"; then',
    '    {',
    "      printf '\\n# >>> herd machine bootstrap >>>\\n'",
    "      printf 'export PATH=\"$HOME/.herd/tools/bin:$PATH\"\\n'",
    "      printf '# <<< herd machine bootstrap <<<\\n'",
    '    } >> "$file"',
    '  fi',
    '}',
  ...
  if (bootstrapOptions.configureRemoteSshHardening) {
    scriptLines.push(
      ...
      'const filePath = process.env.HERD_SSHD_CONFIG_PATH || "/etc/ssh/sshd_config"',
      'const begin = "# >>> herd ssh hardening >>>"',
      'const end = "# <<< herd ssh hardening <<<"',
      'const block = [',
      '  begin,',
      '  "AcceptEnv HERD_APPROVAL_BRIDGE_TOKEN HERD_MACHINE_ENV_*",',
  ...
  if (options.configureTelemetry) {
    ...
    scriptLines.push(
      `printf '%s\\n' ${shellEscape(telemetryJson)} > "$HOME/.herd.json"`,
...
async function runAuthSetup(
  ...
  if (options.mode === 'device-auth' && cliBinaryName) {
    stdout.write(`Next: SSH into the worker and run \`${cliBinaryName} login --device-auth\`, then re-run \`herd machine auth-status --machine `)
...
async function runDaemonPair(
  ...
  stdout.write(`  herd daemon run --machine ${payload.pairing.machineId} --pairing-token ${payload.pairing.token} --endpoint ${normalizeEndpoint(config.endpoint)}\n`)
  stdout.write('\n')
...
async function runBootstrap(
  ...
  // remote sshd without `AcceptEnv HERD_APPROVAL_BRIDGE_TOKEN HERD_MACHINE_ENV_*`.
  ...
  if (/^sshd:skipped:no-sudo$/m.test(commandResult.stdout)) {
    ...
    stdout.write('    Without `AcceptEnv HERD_APPROVAL_BRIDGE_TOKEN HERD_MACHINE_ENV_*`, the\n')
    ...
    stdout.write(`         herd machine bootstrap ${options.id}\n`)
    stdout.write('      2. SSH to the target as root and append to `/etc/ssh/sshd_config`:\n')
    stdout.write('           # >>> herd ssh hardening >>>\n')
    stdout.write('           AcceptEnv HERD_APPROVAL_BRIDGE_TOKEN HERD_MACHINE_ENV_*\n')
  ...
  stdout.write(`- Inspect provider auth: herd machine auth-status --machine ${options.id}\n`)
  for (const provider of listMachineAuthProviders(providers)) {
    for (const mode of provider.machineAuth?.supportedAuthModes ?? []) {
      if (mode === 'device-auth') {
        stdout.write(`- Prepare ${provider.label} device auth: herd machine auth-setup --machine ${options.id} --provider ${provider.id} --mode ${mode}\n`)
        continue
      }

      const secretLabel = mode === 'setup-token' ? 'token' : 'api-key'
      stdout.write(`- Save a ${provider.label} ${mode}: herd machine auth-setup --machine ${options.id} --provider ${provider.id} --mode ${mode} --secret '<${secretLabel}>'\n`)
    }
  }
  stdout.write('- Bootstrap writes `~/.herd.json` plus PATH setup; agent-specific OTEL env merges remain manual.\n')
  return 0
}
