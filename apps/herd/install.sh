...
#
# From a local checkout (apps/herd):
#   ./install.sh
#
...
PRODUCT_NAME="Herd"
NODE_VERSION="${HERD_NODE_VERSION:-22.16.0}"
PNPM_VERSION="${HERD_PNPM_VERSION:-10.23.0}"
...
fail() {
  local fallback_log="${HOME:-/tmp}/.herd/logs/first-boot.log"
  print_repair_hint \
    "Installer failed" \
    "$*" \
...
choose_setup_path() {
  ...
  setup_path_env="${HERD_SETUP_PATH:-}"
  if [[ -n "$setup_path_env" ]]; then
    ...
    case "$normalized" in
      ...
      *) warn "Unknown HERD_SETUP_PATH=$setup_path_env; using Quickstart." ;;
    esac
  fi
...
repo_archive_url() {
  ...
  archive_override="${HERD_ARCHIVE_URL:-}"
...
clone_and_exec_installer() {
  ...
  if [[ -z "${HERD_CHECKOUT_DIR:-}" && ! -e "$default_checkout" && -d "$HOME/Herd/.git" ]]; then
    default_checkout="$HOME/Herd"
  fi

  REPO_URL="${HERD_REPO_URL:-https://github.com/NickGuAI/Herd.git}"
  REPO_REF="${HERD_REPO_REF:-v0.0.3-beta}"
  checkout_override="${HERD_CHECKOUT_DIR:-}"
...
if [ ! -f "$SCRIPT_DIR/package.json" ] \
  ...
  || [ ! -d "$SCRIPT_DIR/../../packages/herd-cli" ]; then
  clone_and_exec_installer "$@"
fi
...
LOCAL_MACHINE_ENV_FILE="${HERD_LOCAL_MACHINE_ENV_FILE:-$HOME/.herd-env}"
BIN_DIR="${HERD_BIN_DIR:-$HOME/.local/bin}"
SHIM_PATH="$BIN_DIR/herd"
DEFAULT_PORT="20001"
...
build_herd() {
  ...
  if auth0_enabled; then
    "$PNPM_BIN" --dir "$REPO_ROOT" --filter herd run build
    return
  fi
...
ensure_local_machine_registry() {
  mkdir -p "$DATA_DIR"
  HERD_DATA_DIR="$DATA_DIR" \
  HERD_LOCAL_MACHINE_ENV_FILE="$LOCAL_MACHINE_ENV_FILE" \
  "$NODE_BIN" <<'NODE'
const fs = require('fs')
...
provider_env_secret() {
  case "$1" in
    claude) printf '%s' "${HERD_CLAUDE_SETUP_TOKEN:-${CLAUDE_CODE_OAUTH_TOKEN:-${ANTHROPIC_API_KEY:-${ANTHROPIC_AUTH_TOKEN:-}}}}" ;;
    codex) printf '%s' "${HERD_CODEX_API_KEY:-${OPENAI_API_KEY:-}}" ;;
    gemini) printf '%s' "${HERD_GEMINI_API_KEY:-${GEMINI_API_KEY:-${GOOGLE_API_KEY:-}}}" ;;
    opencode) printf '%s' "${HERD_OPENCODE_API_KEY:-${OPENCODE_API_KEY:-}}" ;;
    *) return 1 ;;
  esac
}
...
configure_providers() {
  ...
  if [[ "${HERD_CONFIGURE_PROVIDERS:-1}" == "0" ]]; then
    warn "Skipping provider setup because HERD_CONFIGURE_PROVIDERS=0"
    return 0
  fi

  raw_selection="${HERD_PROVIDERS:-}"
