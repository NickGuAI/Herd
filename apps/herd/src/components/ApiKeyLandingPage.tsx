 ...
 * an instance URL (hosted Herd or a self-hosted Herd) and credential.
...
export function ApiKeyLandingPage({
  ...
  async function validateConnection(instanceBaseUrl: string, credential: string): Promise<boolean> {
    const authHeaders = { 'X-Herd-Api-Key': credential }
      ...
      const scanFrame = async () => {
        ...
        try {
          ...
          if (rawValue) {
            ...
            if (!invite) {
              setScannerError('Scanned code is not a Herd mobile invite.')
            } else {
    ...
    try {
      if (isNative) {
        ...
        const res = await fetch(verifyUrl, {
          headers: { 'X-Herd-Api-Key': keyToSubmit },
        })
        if (res.status === 401) {
          setSubmitError('Invalid API key. Check the key and try again.')
          return
