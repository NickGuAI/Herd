...
import { HERD_MODULE_MANIFESTS } from '../server/module-manifest.js'
...
const features: FeatureMapping[] = [
  ...
  },
  {
    ...
    notes: ['Provider-native plan mode or ask-user-question events must normalize into Herd event shapes before UI rendering.'],
  },
  {
  ...
  },
  {
    ...
    userVisible: 'Mobile users access Herd through responsive command-room/org/automations/settings/inbox surfaces and the iOS wrapper project.',
...
const concepts: ConceptMapping[] = [
  ...
  },
  {
    ...
    meaning: 'Provider/account-level ownership record that tells Herd which commander owns an external account.',
  ...
  },
  {
    ...
    userEffect: 'The same WhatsApp person or email thread continues in the same Herd conversation.',
    owners: ['channels'],
  ...
  },
  {
    ...
    files: ['src/types/herd-events.ts', 'modules/agents/messages/planning.ts', 'modules/agents/components/session-message-list/blocks.tsx'],
  },
  {
    id: 'workspace-target',
  ...
  },
  {
    ...
    files: ['apps/herd/scripts (retired)', 'apps/herd/migrations (retired)', 'apps/herd/agents (retired)'],
...
const observations: SourceObservation[] = [
  {
    ...
    kind: 'drift',
    summary: 'Server manifest metadata describes org identity storage under ${HERD_DATA_DIR}/org/identity.json, while the current store default resolves to ${HERD_DATA_DIR}/org.json.',
...
function sourceTreeXml(files: FileEntry[], directories: DirectoryEntry[]): string {
  ...
  const lines = [
    ...
    '<architectureSourceTree app="herd" generatedBy="tools/generate-architecture-indexes.ts">',
...
function runtimeIndexXml(files: FileEntry[]): string {
  const lines = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<architectureRuntimeIndex app="herd" generatedBy="tools/generate-architecture-indexes.ts">',
...
function routeIndexXml(): string {
  const lines = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<architectureRouteIndex app="herd" generatedBy="tools/generate-architecture-indexes.ts">',
    `${indent(1)}<frontendRoutes>`,
  ]
  for (const graph of HERD_MODULE_GRAPH) {
    for (const route of graph.ui.routes ?? []) {
...
function featureIndexXml(): string {
  const lines = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<architectureFeatureIndex app="herd" generatedBy="tools/generate-architecture-indexes.ts">',
    `${indent(1)}<purpose>Map user-visible Herd features to the UI components, hooks, API routes, runtime files, storage keys, tests, and architectural notes behind them.</purpose>`,
  ]
...
function conceptIndexXml(): string {
  const lines = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<architectureConceptIndex app="herd" generatedBy="tools/generate-architecture-indexes.ts">',
    `${indent(1)}<purpose>Define Herd concepts in terms of ownership, user-visible effect, and source files so diagrams and fixes do not mix domains.</purpose>`,
  ]
...
function observationsXml(): string {
  const lines = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<architectureSourceObservations app="herd" generatedBy="tools/generate-architecture-indexes.ts">',
...
function buildVisualizerHtml(files: FileEntry[], directories: DirectoryEntry[]): string {
  ...
  const moduleData = HERD_MODULE_MANIFESTS.map((manifest) => {
...
<head>
  ...
  <title>Herd Architecture Index Visualizer</title>
  <style>
    :root {
      color-scheme: light;
      --bg: #f7f7f4;
...
<body>
  <header>
    <h1>Herd Architecture Index Visualizer</h1>
    <div class="toolbar">
...
function readmeMarkdown(files: FileEntry[], directories: DirectoryEntry[]): string {
  return `# Herd Architecture Indexes
...
- Runtime modules mapped: ${HERD_MODULE_MANIFESTS.length}
- Source observations recorded: ${observations.length}

Regenerate after structural changes with:

\`\`\`bash
pnpm --filter herd exec tsx tools/generate-architecture-indexes.ts
\`\`\`
`
}
