...
import { parseProviderId } from '../modules/agents/providers/registry.js'
import type { AgentType } from '../modules/agents/types.js'
import { resolveCommanderDataDir } from '../modules/commanders/paths.js'
import { resolveAutomationsDataDir } from '../modules/data-dir.js'
import {
...
import type {
  ...
  AutomationStatus,
} from '../modules/automations/types.js'
...
const MEMORY_CONSOLIDATION_CLEANUP_INSTRUCTION = [
  'For each active commander listed via herd commander list (or the commanders API),',
...
function resolveMigrationRoots(automationsDir: string): MigrationRoots {
  const herdDataDir = path.dirname(automationsDir)
  return {
    herdDataDir,
    singularAutomationDir: path.join(herdDataDir, 'automation'),
    commandRoomDir: path.join(herdDataDir, 'command-room'),
    commandRoomBackupDir: path.join(herdDataDir, 'legacy-backup', 'command-room'),
    sentinelsFilePath: path.join(herdDataDir, 'sentinels', 'sentinels.json'),
...
function hasDeadMemoryCompactInstruction(instruction: string): boolean {
  return /\bherd\s+memory\s+compact\b/u.test(instruction)
}
