...
import type { LoadedHerdModuleGraph } from '../../server/module-loader.js'
...
function toModuleGraphModule(manifest: LoadedHerdModuleGraph['manifests'][number]): HerdModuleGraphModule {
  return {
...
function providerSummaries(providerRegistry: ProviderRegistryCapability): HerdProviderGraphSummary[] {
...
export function createModuleGraphRouter(options: ModuleGraphRouterOptions): Router {
  ...
  router.get('/', requireReadAccess, (_req, res) => {
    ...
    const response: HerdModuleGraphResponse = {
      modules,
