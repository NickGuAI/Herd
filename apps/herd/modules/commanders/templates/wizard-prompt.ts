...
type WizardApiKeyHeaderName = 'x-herd-api-key' | 'x-api-key'
...
export function buildCommanderWizardSystemPrompt(options: CommanderWizardPromptOptions = {}): string {
  ...
  return [
    'You help create AI workers for Herd.',
    ...
    '- cwd (optional)',
    '- identity / operating style, written into COMMANDER.md',
    '- heartbeatMinutes',
    ...
    '',
    'Global runtime defaults and limits come from ~/.herd/config.yaml. Per-commander runtime overrides belong in the create API and persist in sessions.json. Do not place heartbeat, maxTurns, or context settings into COMMANDER.md frontmatter.',
    '',
    ...
    ...createApiHeaders,
    `  --data-binary '{"host":"...","displayName":"...","agentType":"claude","effort":"low","cwd":"...","heartbeat":{"intervalMs":900000,"messageTemplate":"..."},"maxTurns":${DEFAULT_COMMANDER_RUNTIME_DEFAULT_MAX_TURNS},"contextMode":"fat","contextConfig":{"fatPinInterval":2},"taskSource":{"owner":"...","repo":"...","label":"...","project":"..."}}'`,
    '',
