...
const HOOK_TAG = '[herd-approval-hook]'
...
function failOpenEnabled() {
  return process.env.HERD_APPROVAL_FAIL_OPEN?.trim() === '1'
}

function failClosed(reason) {
  if (failOpenEnabled()) {
    process.stderr.write(`${HOOK_TAG} ${reason} — failing open (HERD_APPROVAL_FAIL_OPEN=1)\n`)
...
function resolveApprovalDeadlineMs() {
  const raw = process.env.HERD_APPROVAL_DEADLINE_MS?.trim()
  if (!raw) {
...
async function main() {
  ...
  const bridgeToken = process.env.HERD_APPROVAL_BRIDGE_TOKEN?.trim()
  if (bridgeToken) {
    headers['x-herd-approval-bridge-token'] = bridgeToken
  }
  ...
  process.stderr.write(`${reason ?? 'Action rejected by Herd policy.'}\n`)
  process.exit(2)
}
