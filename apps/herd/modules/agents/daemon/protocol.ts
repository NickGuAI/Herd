...
export const HERD_DAEMON_PROTOCOL_VERSION = 1
...
export interface DaemonHelloMessage {
  type: 'hello'
  protocolVersion: typeof HERD_DAEMON_PROTOCOL_VERSION
  machineId: string
...
export function buildDaemonWelcomeMessage(input: {
  ...
  return {
    type: 'welcome',
    protocolVersion: HERD_DAEMON_PROTOCOL_VERSION,
...
function parseHello(record: Record<string, unknown>): DaemonHelloMessage | null {
  if (record.protocolVersion !== HERD_DAEMON_PROTOCOL_VERSION) {
    return null
  }
