...
export const claudeApprovalAdapter = registerApprovalAdapter<ProviderApprovalAdapter<ClaudeApprovalHookEvent, void>>({
  ...
  toUnifiedRequest(rawEvent, session) {
    const sessionName = typeof rawEvent.payload.herd_session_name === 'string'
      && rawEvent.payload.herd_session_name.trim().length > 0
      ? rawEvent.payload.herd_session_name.trim()
      : session.name
