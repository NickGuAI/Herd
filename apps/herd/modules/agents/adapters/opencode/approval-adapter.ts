...
export interface OpenCodeApprovalRawEvent {
...
function pickToolValue(rawEvent: OpenCodeApprovalRawEvent, key: string): unknown {
...
function readToolTitle(rawEvent: OpenCodeApprovalRawEvent): string | undefined {
...
function deriveOpenCodeToolInput(rawEvent: OpenCodeApprovalRawEvent): unknown {
  ...
  if (kind === 'mcp') {
    ...
    // but not the structured MCP server + tool identifiers Herd needs for
...
function readPermissionOptions(rawEvent: OpenCodeApprovalRawEvent): Array<Record<string, unknown>> {
...
export const opencodeApprovalAdapter = registerApprovalAdapter<ProviderApprovalAdapter<OpenCodeApprovalRawEvent, void>>({
  source: 'opencode',
  ...
  toUnifiedRequest(rawEvent, session) {
    ...
    return {
      ...
      toolInput: deriveOpenCodeToolInput(rawEvent),
