...
export function createGeminiUserTranscriptEnvelopes(
  ...
  const events: TranscriptEnvelope[] = [
    createGeminiSyntheticEnvelope('herd/user', { type: 'message.start', role: 'user' }, envelopeOptions),
  ]
  if (visibleText) {
  ...
  events.push(createGeminiSyntheticEnvelope('herd/user', { type: 'message.end' }, envelopeOptions))
  return events
}
