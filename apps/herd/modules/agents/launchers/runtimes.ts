...
import { buildOpenCodeAcpInvocation } from '../adapters/opencode/helpers.js'
import {
...
type OpenCodeNotificationCallback = (message: OpenCodeProtocolMessage) => void

function isSpawnEnoent(error: Error): boolean {
  const code = (error as NodeJS.ErrnoException).code
  return code === 'ENOENT' || /ENOENT/.test(error.message)
}

function toOpenCodeRuntimeProcessError(error: Error): Error {
...
export class GeminiAcpRuntime implements GeminiAcpRuntimeHandle {
  ...
  async ensureConnected(): Promise<void> {
    ...
    await this.sendRequest('initialize', {
      protocolVersion: 1,
      clientInfo: { name: 'herd', version: '0.1.0' },
      clientCapabilities: {
        auth: { terminal: false },
...
export class OpenCodeAcpRuntime implements OpenCodeAcpRuntimeHandle {
  ...
  requestId = 0
  pendingRequests = new Map<number, { resolve: (value: unknown) => void; reject: (error: Error) => void }>()
  notificationListeners = new Map<string, Set<OpenCodeNotificationCallback>>()
  stdoutTail: string[] = []
  stderrTail: string[] = []
  teardownPromise: Promise<void> | null = null
  transportInitialized = false
  ...
  private log(level: 'info' | 'warn' | 'error', message: string, extra: Record<string, unknown> = {}): void {
    ...
    const line = `[agents][opencode-acp] ${message} ${payload}`
    if (level === 'error') {
      console.error(line)
      return
    }
  ...
  private recordOutput(stream: 'stdout' | 'stderr', chunk: Buffer): void {
    ...
    appendCodexSidecarTail(tail, lines)
    if (stream === 'stderr') {
      console.warn(`[agents][opencode-acp][stderr] ${truncateLogText(lines.join(' | '), 800)}`)
    }
  }
  ...
  private handleProtocolMessage(payloadText: string): void {
    try {
      ...
      this.log('warn', 'Failed to parse OpenCode ACP payload', {
        error: truncateLogText(error instanceof Error ? error.message : String(error)),
        payloadSnippet: truncateLogText(payloadText, 800),
      })
    }
  ...
  private attachProcess(cp: ChildProcess): void {
    ...
    cpEmitter.on('exit', (code: number | null, signal: string | null) => {
      const detail = signal
        ? `OpenCode ACP runtime exited with signal ${signal}`
        : `OpenCode ACP runtime exited with code ${code ?? -1}`
      this.log('warn', 'OpenCode ACP runtime process exited', {
        detail,
        exitCode: code ?? -1,
        signal: signal ?? null,
      })
    ...
    cpEmitter.on('error', (error: Error) => {
      const runtimeError = toOpenCodeRuntimeProcessError(error)
      this.log('error', 'OpenCode ACP runtime process error', {
        error: truncateLogText(runtimeError.message),
      })
      this.process = null
      this.transportInitialized = false
  ...
  private sendTransportPayload(payloadText: string): void {
    const stdin = this.process?.stdin
    if (!stdin || stdin.writable === false) {
      throw new Error('OpenCode ACP runtime not connected')
    }
    stdin.write(payloadText + '\n')
  }
    ...
    if (!this.process) {
      ...
      const cp = this.machine
        ...
        : spawn(
          OPENCODE_ACP_COMMAND,
          this.model
            ? [...OPENCODE_ACP_ARGS, '--model', this.model]
            : OPENCODE_ACP_ARGS,
          {
            stdio: ['pipe', 'pipe', 'pipe'],
            env: { ...process.env, ...this.providerAuth?.env },
  ...
  addNotificationListener(sessionId: string, cb: OpenCodeNotificationCallback): () => void {
    if (!this.notificationListeners.has(sessionId)) {
      this.notificationListeners.set(sessionId, new Set())
    }
    this.notificationListeners.get(sessionId)!.add(cb)
  ...
  async teardown(options: { reason?: string; timeoutMs?: number } = {}): Promise<void> {
    ...
    this.teardownPromise = (async () => {
      const reason = options.reason ?? 'OpenCode ACP teardown'
      const timeoutMs = options.timeoutMs ?? CODEX_RUNTIME_TEARDOWN_TIMEOUT_MS
      this.transportInitialized = false
      this.notificationListeners.clear()
      this.rejectPendingRequests(new Error(reason))
      ...
      this.log('warn', 'OpenCode ACP runtime did not exit after SIGTERM; escalating to SIGKILL', { timeoutMs })
      try {
        runtimeProcess.kill('SIGKILL')
      } catch {
        // Process may have exited.
