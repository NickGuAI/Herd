...
const SSH_CONTROL_DIR = path.join(resolveHerdDataDir(), 'ssh-control')
...
export function defaultMachineRegistryStorePath(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveHerdDataDir(env), 'machines.json')
}
 ...
 *
 *   -o SendEnv=HERD_APPROVAL_BRIDGE_TOKEN
