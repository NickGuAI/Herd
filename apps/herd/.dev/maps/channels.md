...
- channels: provider account bindings, runtime manager state, channel provider
  descriptors, inbound provider config, surface binding records, and provider
  auth material references.
- commanders: conversation records, `channelMeta`, `lastRoute`,
  `channelReplyIntents`, `channelReplyDelivery`, and
...
Assistant turn writes transcript events
        ...
        |
        v
mapStreamEventsToMessages projects Command Room messages
        |
        v
...
- provider adapter/runtime: account startup, inbound normalization, outbound
  `send`, provider-specific failure reporting.
- channel resolver and surface-binding store: commander target, conversation
  target, `sessionKey`, and peer identity.
- commander conversation runtime: create/resume, `sendToSession`, durable
  ...
  completed-turn skipping, delivery-state writes.
...
- delivery state: `channelReplyDelivery`, `channelReplyIntents`, `channelMeta`,
  `lastRoute`, and surface binding must stay coherent after reload/resume. A
  pending delivery is blocking only while an in-flight send owns it or a pending
  intent references its delivery id; orphan pending delivery slots must not block
  newer intents.
