...
function storage(
  ...
  kind: HerdStorageOwnership['kind'],
  notes: string,
  fields: Partial<Omit<HerdStorageOwnership, 'kind' | 'ownerModuleId' | 'notes'>> = {},
): HerdStorageOwnership {
  return {
    kind,
    ownerModuleId,
...
export const HERD_MODULE_SERVER_METADATA = [
  {
    id: 'agents',
    directory: 'agents',
    serverOnly: true,
  ...
  },
  {
    ...
    storage: storage('api-keys', 'owned', 'API keys and provider secrets are server-side credential data.', {
      ...
      roots: ['${HERD_DATA_DIR}/api-keys', '${HERD_DATA_DIR}/settings'],
      files: ['keys.json'],
    }),
    dependencies: HERD_MODULE_GRAPH[1].dependencies,
    capabilities: HERD_MODULE_GRAPH[1].capabilities,
  },
  {
    id: 'approvals',
    directory: 'approvals',
    ...
    storage: storage('approvals', 'shared', 'Approval queue and audit files are currently implemented by the policies pending store.', {
      ...
      roots: ['${HERD_DATA_DIR}/policies'],
      files: ['pending.json', 'audit.jsonl'],
      sharedWith: ['policies'],
    }),
    dependencies: HERD_MODULE_GRAPH[2].dependencies,
    capabilities: HERD_MODULE_GRAPH[2].capabilities,
  },
  {
    id: 'automations',
    ...
    storage: storage('automations', 'owned', 'Automation definitions and run artifacts live under the automation store root.', {
      ...
      roots: ['${HERD_DATA_DIR}/automations'],
      files: ['*.json', 'runs/*.json', 'runs/*.md', 'memory.md'],
    }),
    dependencies: HERD_MODULE_GRAPH[3].dependencies,
    capabilities: HERD_MODULE_GRAPH[3].capabilities,
  },
  {
    id: 'channels',
    directory: 'channels',
    ...
    dependencies: HERD_MODULE_GRAPH[4].dependencies,
    capabilities: HERD_MODULE_GRAPH[4].capabilities,
  },
  {
    id: 'command-room',
    ...
    dependencies: HERD_MODULE_GRAPH[5].dependencies,
    capabilities: HERD_MODULE_GRAPH[5].capabilities,
  },
  {
    id: 'commanders',
    ...
    storage: storage('commanders', 'owned', 'Commander profile, memory, quest, heartbeat, and transcript data share the commander data root.', {
      ...
      roots: ['${HERD_DATA_DIR}/commander'],
      files: ['sessions.json', 'names.json', '<commander-id>/**'],
    }),
    dependencies: HERD_MODULE_GRAPH[6].dependencies,
    capabilities: HERD_MODULE_GRAPH[6].capabilities,
  },
  {
    id: 'components',
    ...
    dependencies: HERD_MODULE_GRAPH[7].dependencies,
    capabilities: HERD_MODULE_GRAPH[7].capabilities,
  },
  {
    id: 'conversation',
    ...
    dependencies: HERD_MODULE_GRAPH[8].dependencies,
    capabilities: HERD_MODULE_GRAPH[8].capabilities,
  },
  {
    id: 'onboarding',
    ...
    dependencies: HERD_MODULE_GRAPH[9].dependencies,
    capabilities: HERD_MODULE_GRAPH[9].capabilities,
  },
  {
    id: 'operators',
    directory: 'operators',
    ...
    storage: storage('operators', 'owned', 'Operator profiles and avatars are stored by the operator module.', {
      ...
      roots: ['${HERD_DATA_DIR}/operators'],
      files: ['operators.json', 'avatars/**'],
    }),
    dependencies: HERD_MODULE_GRAPH[10].dependencies,
    capabilities: HERD_MODULE_GRAPH[10].capabilities,
  },
  {
    id: 'org',
    directory: 'org',
    ...
    dependencies: HERD_MODULE_GRAPH[11].dependencies,
    capabilities: HERD_MODULE_GRAPH[11].capabilities,
  },
  {
    id: 'org-identity',
    ...
    dependencies: HERD_MODULE_GRAPH[12].dependencies,
    capabilities: HERD_MODULE_GRAPH[12].capabilities,
  },
  {
    id: 'policies',
    directory: 'policies',
    ...
    dependencies: HERD_MODULE_GRAPH[13].dependencies,
    capabilities: HERD_MODULE_GRAPH[13].capabilities,
  },
  {
    id: 'quests',
    directory: 'quests',
    ...
    dependencies: HERD_MODULE_GRAPH[14].dependencies,
    capabilities: HERD_MODULE_GRAPH[14].capabilities,
  },
  {
    id: 'rpg',
    directory: 'rpg',
    ...
    dependencies: HERD_MODULE_GRAPH[15].dependencies,
  ...
  },
  {
    ...
    websockets: [],
    lifecycle: noLifecycle(),
    storage: storage('eval', 'owned', 'Benchmark run manifests, summaries, trajectories, and submission state are internal Herd artifacts.', {
...
const serverMetadataById: ReadonlyMap<string, HerdModuleServerMetadata> = new Map(
  HERD_MODULE_SERVER_METADATA.map((metadata) => [metadata.id, metadata]),
)

function getServerMetadata(moduleId: string): HerdModuleServerMetadata {
...
export const HERD_MODULE_MANIFESTS = HERD_MODULE_GRAPH.map((graph) => ({
  graph,
  server: getServerMetadata(graph.id),
})) satisfies readonly HerdModuleManifest[]

export const HERD_MODULE_MANIFEST_BY_ID = new Map(
  HERD_MODULE_MANIFESTS.map((manifest) => [manifest.graph.id, manifest]),
)
