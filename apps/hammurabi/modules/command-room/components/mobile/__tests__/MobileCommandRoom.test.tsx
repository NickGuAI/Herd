...
describe('CommandRoom mobile branch', () => {
  beforeEach(() => {
    ...
    mocks.useCommander.mockReturnValue({
      ...
      setSelectedCommanderId: mocks.setSelectedCommanderId,
      crons: [],
      cronsLoading: false,
      cronsError: null,
  ...
  it('renders the inbox tab from the route when ?surface=mobile is set', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(window.location.pathname).toBe('/command-room/inbox')
  ...
  it('renders the settings tab from the route when ?surface=mobile is set', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(window.location.pathname).toBe('/command-room/settings')
      expect(new URLSearchParams(window.location.search).get('surface')).toBe('mobile')
      expect(new URLSearchParams(window.location.search).get('panel')).toBeNull()
    })
  })
  ...
  it('normalizes exact /command-room?surface=mobile to global automation search on the root path', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(params.get('surface')).toBe('mobile')
      expect(params.get('commander')).toBe('global')
      expect(params.get('panel')).toBe('automation')
    })
  })

  it('keeps global automation URLs on the command-room root without mounting mobile automations', async () => {
    await renderAt('/command-room?surface=mobile&commander=global&panel=automation')
  ...
  it('normalizes ?commander=global to the automation panel on mobile', async () => {
    await renderAt('/command-room?surface=mobile&commander=global')
  ...
  it('routes the mobile chat back chevron to exact /org without stale command-room params', async () => {
    const conversation = buildConversation({
      id: 'conv-1',
      status: 'active',
      liveSession: {
        id: 'session-conv-1',
        name: 'commander-cmd-1-conversation-conv-1',
        status: 'active',
        transportType: 'stream',
        processAlive: true,
        agentType: 'claude',
    ...
    await vi.waitFor(() => {
      expect(document.body.querySelector('[aria-label="Back to org"]')).not.toBeNull()
    })

    document.body.querySelector<HTMLButtonElement>('[aria-label="Back to org"]')?.click()
  ...
  it('MobileCommandRoom is a normal flex-fill (no fixed-overlay) so BottomNav stays tappable', async () => {
    ...
    await renderAt('/command-room?surface=mobile&commander=cmd-1')
