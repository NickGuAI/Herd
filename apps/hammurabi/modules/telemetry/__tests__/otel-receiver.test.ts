...
async function createServer(): Promise<RunningServer & { tmpDir: string }> {
  ...
  const hub = new TelemetryHub({ store, now, retentionDays: 0 })
...
describe('OTEL receiver', () => {
  ...
  afterEach(async () => {
    await server.close()
    await rm(server.tmpDir, { recursive: true, force: true, maxRetries: 10, retryDelay: 100 })
  })
  ...
  it('hub restore re-normalizes otel_log entries from raw data (backfills cost)', async () => {
    ...
    const hub2 = new HubClass({ store: store2, now: () => TEST_NOW, retentionDays: 0 })
