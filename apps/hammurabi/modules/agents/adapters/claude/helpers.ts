...
function resolveApprovalDeadlineMs(){const raw=process.env.HAMMURABI_APPROVAL_DEADLINE_MS?.trim();if(!raw){return DEFAULT_APPROVAL_DEADLINE_MS;}const parsed=Number.parseInt(raw,10);if(!Number.isFinite(parsed)||parsed<=0){return DEFAULT_APPROVAL_DEADLINE_MS;}return parsed;}
