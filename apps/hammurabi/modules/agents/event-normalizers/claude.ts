...
export function createClaudeUserTranscriptEnvelopes(
  ...
  const events: TranscriptEnvelope[] = [
    createClaudeSyntheticEnvelope('hammurabi/user', { type: 'message.start', role: 'user' }, envelopeOptions),
  ]
  if (visibleText) {
  ...
  events.push(createClaudeSyntheticEnvelope('hammurabi/user', { type: 'message.end' }, envelopeOptions))
  return events
}
