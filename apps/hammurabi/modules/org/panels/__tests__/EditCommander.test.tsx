...
describe('EditCommander', () => {
  beforeEach(() => {
    ...
    mocks.generateCommanderAvatar.mockReset()
  ...
  it('renders Generate avatar, opens the confirm dialog, refreshes the preview, and updates the org cache', async () => {
    const queryClient = new QueryClient({
      defaultOptions: {
        queries: { retry: false },
        mutations: { retry: false },
    ...
    const invalidateQueries = vi.spyOn(queryClient, 'invalidateQueries').mockResolvedValue(undefined)
    const orgQueryKey = [...ORG_QUERY_KEY, { includeArchived: false }] as const
    queryClient.setQueryData(orgQueryKey, createOrgTree())
    vi.spyOn(Date, 'now').mockReturnValue(123456789)

    mocks.fetchOrgCommanderDetail
    ...
    mocks.generateCommanderAvatar.mockResolvedValue({
      avatarUrl: '/api/commanders/cmd-1/avatar',
    })
    ...
    await act(async () => {
      getButtonByText('Generate now').dispatchEvent(new MouseEvent('click', { bubbles: true }))
    })
    await flushReact()
  ...
  it('shows the sanitized route error when avatar generation fails', async () => {
    mocks.fetchOrgCommanderDetail.mockResolvedValue({
      id: 'cmd-1',
      displayName: 'Atlas',
      avatarUrl: '/api/commanders/cmd-1/avatar',
    ...
    mocks.generateCommanderAvatar.mockRejectedValue(
      new Error(
        'Request failed (412): {"error":"Configure Gemini API key in Settings → Image Generation"}',
      ),
    )
    ...
    await vi.waitFor(() => {
      expect(document.body.querySelector('[data-testid="edit-commander-avatar-error"]')?.textContent).toContain(
        'Configure Gemini API key in Settings → Image Generation',
      )
    })
  })
})
