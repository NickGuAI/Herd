...
describe('commander memory module facade', () => {
  ...
  it('builds commander session seeds through the extracted memory boundary', async () => {
    ...
    expect(built.systemPrompt).toContain('# Global Rules Bootstrap')
    expect(built.systemPrompt).toContain('# Shared Knowledge Bootstrap')
    expect(built.systemPrompt).toContain('## Commander Memory')
    expect(built.systemPrompt).toContain('### Progressive Memory Discovery')
    expect(built.systemPrompt).toContain('**Issue #77**: Issue #77 — NickGuAI/example-repo')
    expect(built.systemPrompt).not.toContain('Prefer deterministic fixes.')
    expect(built.systemPrompt).not.toContain('Prior note.')
  })
})
