#!/usr/bin/env tsx
import { inspectHammurabiDatabaseReadiness } from '../server/db/readiness.js'

function usage(): string {
  return [
    'Usage: pnpm --filter hammurabi run db:ready -- [--source-root <dir>] [--db <file>] [--no-init]',
    '',
    'Checks the Hammurabi SQLite runtime-session database. Fresh installs initialize by default.',
  ].join('\n')
}
