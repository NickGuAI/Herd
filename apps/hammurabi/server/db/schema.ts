...
export const HAMMURABI_SQLITE_SCHEMA_VERSION = '002_agent_runtime_session_payload'
...
export function hasCurrentHammurabiSqliteSchemaColumns(db: DatabaseSync): boolean {
  const columns = readAgentRuntimeSessionColumns(db)
  if (columns.size === 0) {
    return false
  }
...
export function applyHammurabiSqliteSchema(
  db: DatabaseSync,
...
export function readAppliedHammurabiSchemaVersions(db: DatabaseSync): string[] {
  const table = db.prepare(
...
export function isHammurabiSqliteSchemaCurrent(db: DatabaseSync): boolean {
  return readAppliedHammurabiSchemaVersions(db).includes(HAMMURABI_SQLITE_SCHEMA_VERSION)
    && hasCurrentHammurabiSqliteSchemaColumns(db)
}

export function canUpgradeHammurabiSqliteSchemaInPlace(db: DatabaseSync): boolean {
  const versions = readAppliedHammurabiSchemaVersions(db)
  return versions.includes(AGENT_RUNTIME_SESSIONS_V1_SCHEMA_VERSION)
    || versions.includes(HAMMURABI_SQLITE_SCHEMA_VERSION)
}
